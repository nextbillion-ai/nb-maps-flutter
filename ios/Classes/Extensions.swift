...
extension NGLMapCamera {
    func toDict(mapView: NGLMapView) -> [String: Any] {
        let zoom = NGLZoomLevelForAltitude(
            altitude,
            pitch,
    ...
    static func fromDict(_ dict: [String: Any], mapView: NGLMapView) -> NGLMapCamera? {
        ...
        let altitude = NGLAltitudeForZoomLevel(zoom, tilt, location.latitude, mapView.frame.size)
        return NGLMapCamera(
...
extension NGLCoordinateBounds {
    func toArray() -> [[Double]] {
        return [sw.toArray(), ne.toArray()]
    }

    static func fromArray(_ array: [[Double]]) -> NGLCoordinateBounds {
        let southwest = CLLocationCoordinate2D.fromArray(array[0])
        let northeast = CLLocationCoordinate2D.fromArray(array[1])
        return NGLCoordinateBounds(sw: southwest, ne: northeast)
    }
}
