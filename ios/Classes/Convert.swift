...
class Convert {
    class func interpretNextbillionMapOptions(options: Any?, delegate: NextbillionMapOptionsSink) {
        guard let options = options as? [String: Any] else { return }
        if let cameraTargetBounds = options["cameraTargetBounds"] as? [[[Double]]] {
            delegate
                .setCameraTargetBounds(bounds: NGLCoordinateBounds.fromArray(cameraTargetBounds[0]))
        }
    ...
    class func parseCameraUpdate(cameraUpdate: [Any], mapView: NGLMapView) -> NGLMapCamera? {
        ...
        case "newCameraPosition":
            ...
            return NGLMapCamera.fromDict(cameraPosition, mapView: mapView)
        case "newLatLng":
        ...
        case "newLatLngBounds":
            ...
            return mapView.cameraThatFitsCoordinateBounds(
                NGLCoordinateBounds.fromArray(bounds),
    ...
    class func getZoom(mapView: NGLMapView) -> Double {
        return NGLZoomLevelForAltitude(
    ...
    class func getAltitude(zoom: Double, mapView: NGLMapView) -> Double {
        return NGLAltitudeForZoomLevel(
            zoom,
            mapView.camera.pitch,
            mapView.camera.centerCoordinate.latitude,
            mapView.frame.size
