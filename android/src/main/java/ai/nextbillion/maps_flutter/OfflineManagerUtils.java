...
import ai.nextbillion.maps.offline.OfflineRegion;
import ai.nextbillion.maps.offline.OfflineRegionDefinition;
import ai.nextbillion.maps.offline.OfflineRegionError;
import ai.nextbillion.maps.offline.OfflineRegionStatus;
...
abstract class OfflineManagerUtils {
  ...
  static void setOfflineTileCountLimit(MethodChannel.Result result, Context context, long limit) {
    OfflineManager.getInstance(context).setOfflineNbmapTileCountLimit(limit);
    result.success(null);
  }
        ...
        .createOfflineRegion(
            ...
            new OfflineManager.CreateOfflineRegionCallback() {
              ...
              public void onCreate(OfflineRegion offlineRegion) {
                ...
                OfflineRegion.OfflineRegionObserver observer =
                    new OfflineRegion.OfflineRegionObserver() {
                      ...
                      @Override
                      public void nbmapTileCountLimitExceeded(long limit) {
                        Log.e(TAG, "NBMap tile count" + " limit exceeded: " + limit);
                        // Reset downloading state
                        _offlineRegion.setDownloadState(OfflineRegion.STATE_INACTIVE);
                        isComplete.set(true);
                        channelHandler.onError(
                            "nbMapTileCountLimitExceeded",
                            "nbMap tile count " + "limit " + "exceeded: " + limit,
                            null);
                        // Nbmaps even after crash
              ...
              public void onError(String error) {
                ...
                // Reset downloading state
                _offlineRegion.setDownloadState(OfflineRegion.STATE_INACTIVE);
                channelHandler.onError("nbMapInvalidRegionDefinition", error, null);
                result.error("nbMapInvalidRegionDefinition", error, null);
              }
            });
  }
