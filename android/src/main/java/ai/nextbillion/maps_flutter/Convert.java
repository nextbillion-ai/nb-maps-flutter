...
class Convert {
  ...
  static CameraUpdate toCameraUpdate(Object o, NextbillionMap nextbillionMap, float density) {
  ...
  static Polygon interpretListLatLng(List<List<LatLng>> geometry) {
    ...
    for (List<LatLng> innerGeometry : geometry) {
      List<Point> innerPoints = new ArrayList<>(innerGeometry.size());
  ...
  private static android.graphics.Point toPoint(Object o, float density) {
    final List<?> data = toList(o);
    return new android.graphics.Point(toPixels(data.get(0), density), toPixels(data.get(1), density));
  }
  ...
  static void interpretNextbillionMapOptions(Object o, ai.nextbillion.maps_flutter.NextbillionMapOptionsSink sink, Context context) {
