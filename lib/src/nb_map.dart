...
class NBMap extends StatefulWidget {
  ...
  /// True if you want to be notified of map camera movements by the NextBillionMapController. Default is false.
  ///
  /// If this is set to true and the user pans/zooms/rotates the map, NextBillionMapController (which is a ChangeNotifier)
  /// will notify it's listeners and you can then get the new NextBillionMapController.cameraPosition.
  ...
  /// Set the layout margins for the Nbmaps Logo
  final Point? logoViewMargins;

  /// Set the position for the Nbmaps Compass
  final CompassViewPosition? compassViewPosition;

  /// Set the layout margins for the Nbmaps Compass
  final Point? compassViewMargins;

  /// Set the position for the Nbmaps Attribution Button
  final AttributionButtonPosition? attributionButtonPosition;

  /// Set the layout margins for the Nbmaps Attribution Buttons. If you set this
  ...
  static bool get useHybridComposition =>
      MethodChannelNbMapsGl.useHybridComposition;
  static set useHybridComposition(bool useHybridComposition) =>
      MethodChannelNbMapsGl.useHybridComposition = useHybridComposition;

  @override
  State createState() => _NBMapState();
}

class _NBMapState extends State<NBMap> {
  ...
  Widget build(BuildContext context) {
    ...
    return _nbmapsGlPlatform.buildView(
  ...
  void _updateOptions(Map<String, dynamic> updates) async {
    ...
    final NextbillionMapController controller = await _controller.future;
    controller._updateMapOptions(updates);
  }

  Future<void> onPlatformViewCreated(int id) async {
    final NextbillionMapController controller = NextbillionMapController(
      nbMapsGlPlatform: _nbmapsGlPlatform,
